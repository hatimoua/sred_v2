...
def test_validate_proof_spans_success():
    ...
    spans = [
        ProofSpan(start=4, end=23, excerpt="system architecture", kind="architecture"),
        ProofSpan(start=24, end=39, excerpt="uses a database", kind="constraint")
    ]
    valid, errors = validate_proof_spans(text, spans)
...
def test_validate_proof_spans_offset_mismatch():
    ...
    spans = [ProofSpan(start=0, end=5, excerpt="Fixed", kind="error")]
    valid, errors = validate_proof_spans(text, spans)
    assert valid is False
    assert "excerpt mismatch" in errors[0]

def test_validate_proof_spans_out_of_bounds():
    text = "Short."
    spans = [ProofSpan(start=0, end=10, excerpt="Too long", kind="error")]
    valid, errors = validate_proof_spans(text, spans)
    assert valid is False
...
def test_validate_proof_spans_too_short_no_marker():
    ...
    spans = [ProofSpan(start=2, end=6, excerpt="tiny", kind="other")]
    valid, errors = validate_proof_spans(text, spans)
    assert valid is False
...
def test_validate_proof_spans_short_with_marker():
    ...
    spans = [ProofSpan(start=6, end=11, excerpt="ERROR", kind="error")]
    valid, errors = validate_proof_spans(text, spans)
    assert valid is True

def test_decide_final_state_noise():
    result = RouterResult(label=RouterLabel.NOISE, confidence=1.0)
    state, risks = decide_final_state(result, "some text")
    assert state == RecommendedState.NOISE

def test_decide_final_state_financial():
    result = RouterResult(label=RouterLabel.FINANCIAL, confidence=0.9)
...
def test_decide_final_state_technical_promotion_success():
    ...
    result = RouterResult(
        ...
        proof_spans=[ProofSpan(start=0, end=9, excerpt="Traceback", kind="error")]
    )
...
def test_decide_final_state_technical_invalid_proof():
    ...
    result = RouterResult(
        ...
        recommended_state=RecommendedState.INDEX_READY,
        proof_spans=[ProofSpan(start=0, end=5, excerpt="Wrong", kind="architecture")]
    )
    state, risks = decide_final_state(result, text)
    assert state == RecommendedState.REVIEW
...
def test_decide_final_state_technical_no_signals():
    ...
    result = RouterResult(
        ...
        signals=[],
        recommended_state=RecommendedState.INDEX_READY,
        proof_spans=[ProofSpan(start=0, end=10, excerpt="Generic te", kind="other")]
    )
    state, risks = decide_final_state(result, text)
    assert state == RecommendedState.REVIEW
...
def test_decide_final_state_technical_low_confidence():
    ...
    result = RouterResult(
        ...
        signals=[HardSignal.ARCHITECTURE_COMPONENT],
        recommended_state=RecommendedState.INDEX_READY,
        proof_spans=[ProofSpan(start=0, end=12, excerpt="Architecture", kind="architecture")]
    )
    state, risks = decide_final_state(result, text)
    assert state == RecommendedState.REVIEW
